...
public class SellingListActivity extends AppCompatActivity {
    ...
    String TAG = "SellingListActivity";
    HomeRCAdapter adapter;
    RadioGroup radioGroup;
    RadioButton allList, selling, soldout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 라디오그룹의 초기값을 설정해주고, 아이템 체인지 리스너를 달아둔 메소드
        setRadioGroup();

        // 어뎁터를 객체화 하고, 리사이클러뷰를 만들어준 다음, 리사이클러뷰 어뎁터를 객체화한 어뎁터로 넣음.
        adapter = new HomeRCAdapter(this);

        RecyclerView recyclerView = findViewById(R.id.recyclerView_sellinglist);
        ...
        endlessScrollEventListener = new EndlessScrollEventListener(linearLayoutManager) {
            ...
            public void onLoadMore(int pageNum, RecyclerView recyclerView) {
                ...
                // 서버에 데이터를 더 요청하는 부분
                loadSellingList(pageNum, mStatus);
            }
        };
    ...
    protected void onStart() {
        ...
        loadSellingList(0, mStatus);
    }

    public void loadSellingList(int pageNum, final String status){
        Log.d(TAG, "loadSellingList");
        ...
        Call<ResponseBody> call = retrofitService.sellingList(UserInfo.phoneNum, pageNum, status);
                ...
                if(response.isSuccessful()){
                    ...
                    try {
                        ...
                        for(int i = 0 ; i < jsonArray.length() ; i++){
                            ...
                            HomeRCData item = new HomeRCData(no, image, phonenum, title, address, time, category, price + " 원", comment, marked, mStatus);
    ...
    public void setRadioGroup(){
        // 초기 값을 설정해줌.
        allList.setChecked(true);

        // 값이 변경될 때마다, 리사이클러뷰에 보이는 데이터를 바꿔줌.
        radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                switch (checkedId){
                    // 체크된 아이디 값이 바뀌면, 어뎁터와 스크롤리스너를 초기화해주고, 다시 서버에서 받아서 게시물들을 뿌려준다.
                    case R.id.allList:
                        mStatus = "all";
                        adapter.items.clear();
                        endlessScrollEventListener.reset();
