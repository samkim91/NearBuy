...
public class EditActivity extends AppCompatActivity {
    ...
    ImageView addBtn;
    TextView imageNum;
    EditText mTitle, mPrice, mContent;
    Spinner category_spinner;

    // 현재 값들
    String cTitle, cCategory, cContent;
    int cNo, cPrice;
    ArrayList<String> cImageUriList = new ArrayList<>();

    // 스피너에서 선택된 값
    String selectedCategory;

    // 앨범에서 사진을 가져올 때 사용하는 코드와 사진의 path를 담는 어레이리스트
    int REQUEST_CODE_FOR_IMAGES = 0;
    ArrayList<String> imagePath = new ArrayList<>();

    // Write 부분과 Edit 부분에서 쓰는 리사이클러뷰가 같기 때문에 같은 아이템 클래스로 생성함. 앞으로도 자원을 공유할 예정.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cNo = intent.getIntExtra("no", 0);
        cTitle = intent.getStringExtra("title");
        cCategory = intent.getStringExtra("category");
        cContent = intent.getStringExtra("content");
        cPrice = intent.getIntExtra("price", 0);
        cImageUriList = intent.getStringArrayListExtra("imageUriList");


        getAddressFromSharedPreferences();
//        Log.d(TAG, String.valueOf(cImageUriList));
        ...
        // 인텐트로 받아온 이미지 uri를 통해서 리사이클러뷰에 초기 이미지를 뿌려주는 부분.
        for(int i=0 ; i<cImageUriList.size() ; i++){
            WriteRCData item = new WriteRCData(cImageUriList.get(i));
...
//        String formattedPrice = decimalFormat.format(price);
        ...
        // 에딭텍스트 및 스피너 받아온 값 넣기
        mTitle.setText(cTitle);
        mContent.setText(cContent);
        mPrice.setText(String.valueOf(cPrice));

        // 스피너에 대한 초기화와 기능이 들어가 있는 메소드
        category_spinner_method();
        ...
        // 리사이클러뷰 아이템마다 붙어있는 삭제 버튼을 눌렀을 때, 해당 아이템이 삭제되는 기능
        adapter.setOnItemClickListener(new OnWriteRCClickListener() {
            ...
            public void onItemClick(WriteRCAdapter.ViewHolder viewHolder, View view, int position) {
                ...
                // 현재 사진들의 uri 주소도 서버로 보내서 저장할 때 활용할 예정이므로 같이 삭제해줌. 이때, 위치가 imageurilist보다 작은지를 검사하는 이유는,
                // 기존 이미지들은 어뎁터 리스트 중에 앞쪽에 배치될 것이고, 새로운 이미지들은 뒤쪽에 추가될 예정이므로, 선택한 위치가 imageurilist보다 작다는 것은 기존 사진을 삭제했다는 것.
                if(position<cImageUriList.size()){
                    cImageUriList.remove(position);
                }else{
                    // 이보다 크다는 것은 새로운 사진을 삭제했다는 것이기 때문에 어뎁터에서 삭제한 것에 이어서 새로 추가된 이미지path 어레이에서도 포지션을 없에줘야함.
                    int i = position - cImageUriList.size();
                    imagePath.remove(i);
                }
    ...
    public void edit(){
        ...
        // TODO.. 위치, 사진 넣어야함..)
        hashMap.put("no", String.valueOf(cNo));
//        hashMap.put("location", UserInfo.location);
        ...
        hashMap.put("price", mPrice.getText().toString());
        hashMap.put("content", mContent.getText().toString());
        // 앨범에서 가져온 이미지는 위에서 이미지 리스트로 만들어서 보내고, 편집과정에서도 남아있는 이미지는 uri를 다시 서버로 가져간다.
        // 서버단에서 imageUriList와 서버로 새로 보낸 이미지의 이름 리스트를 합쳐서 DB에 저장한다.
        hashMap.put("imageUriList", cImageUriList);
        ...
        Call<ResponseBody> call = retrofitService.editPost(imageList, hashMap, hashMap1);
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                Toast.makeText(getApplicationContext(), "서버 접근 실패", Toast.LENGTH_SHORT).show();
                Log.d(TAG, "the reason failed: "+t);
            }
        });
    }
    ...
    public void category_spinner_method(){
        ...
        // 스피너의 현재 값을 확인하고, 그 값에 따라 초기 값을 설정한다.
        int cLocationOfSpinner = adapter.getPosition(cCategory);
        category_spinner.setSelection(cLocationOfSpinner);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        switch (item.getItemId()){
            case R.id.post_btn:
                // 빈칸 검사
                if(!adapter.items.isEmpty() && !mTitle.getText().toString().equals("") && !mPrice.getText().toString().equals("") && !mContent.getText().toString().equals("")){
                    // 카테고리 선택 검사
                    ...
                    Toast.makeText(getApplicationContext(), "빈칸이나 사진을 채워주세요!", Toast.LENGTH_SHORT).show();
                }
                return true;
            default:
