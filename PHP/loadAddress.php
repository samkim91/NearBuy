...
<script>
  ...
  function onReceivedPostMessage(event){
       ...
       console.log("onReceivedPostMessage(), event=", event);
  }
      ...
      function sample2_execDaumPostcode() {
          new daum.Postcode({
              oncomplete: function(data) {
  				...
  				window.Android.processDATA(data.bname + "/" + fullRoadAddr);
              },
              width : '100%',
              height : '100%'
          }).embed(element_layer);
  ...
  new daum.Postcode({
    oncomplete: function(data){
      if(data.userSelectedType == "R"){
        // userSelectedType : 검색결과에서 사용자가 선택하는 주소의 타입.
        // R - 도로명 주소, J - 지번 주소

        // Traidng 은 안드로이드에서 등록한 이름.(앱의 이름으로 등록해놓음)
        window.Trading.setAddress(data.zonecode, data.roadAddress, data.buildingName);
      }else{
        window.Trading.setAddress(data.zonecode, data.jibunAddress, data.buildingName);
      }
    }
  }).embed();
